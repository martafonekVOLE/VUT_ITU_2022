...
function generateIngredientDiv(name) {
    return `        
...
function generateStepDiv(id, name, description, time) {
    const step_id = "step_" + id;
    let htmlSegment = `
            <div id=${step_id} class="step">
                <h2 class="step_name">${name}</h2>
                <div class="step-contents">
                <p class="step_description">${description}</p>`;
    ...
    if (time === -1) {
        ...
        htmlSegment +=`
            <svg class="timer-icon" onclick="getNewTimer(${id}, ${time}, '${name}')" width="3em" height="3em"  version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 296.228 296.228" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 296.228 296.228">
              <g>
...
async function renderRecipe(recipeId) {
    ...
    let htmlResponse = `<div class="recipe-detail"><h1>${recipe.name}</h1>
                        <h2 class="ingredient-header">Nachystej si ingredience</h2>
    ...
    htmlResponse += `<div class="timer_container" id="timer_container"></div></div>`;
